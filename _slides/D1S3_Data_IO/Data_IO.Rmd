...
`readr` is a `tidyverse` package that provides convenient functions to turn *flat* data files into data frames (`tibble`s to be precise):
...
Assume we have a *flat* data set with variables `id`, `var_1`, and `var_2` and cases as rows. To read in such data use `read_csv()`, which will automatically identify (a) columns and rows, (b) column names, (c) the type of the columns and finally return a `tibble` (more on that later).
...
Behind the magic of `readr` functions such as `read_csv` are a set of really flexible at parsing functions `parse_*()` that transform the input into the apprpriate format and type. 
...
How the text input is parsed can usually be controlled using function arguments. If no function arguments are provided the parsing behavior is controlled by `locale()`. To change, e.g., the `decimal_mark` use `locale("en", decimal_mark = ",")`.
...
When reading in a file using, e.g., `read_csv`, with no specific arguments provided, `readr` infers the type of each column using a heuristic process base on the first 1,000 rows (see `guess_parser`). However, that may not always work. In that case consider ...  
...
The `tidyverse` together with other package offers a variety of build-in, automated read functions for almost any data format. For an overview see [rio](https://cran.r-project.org/web/packages/rio/vignettes/rio.html).
.pull-left8[
...
.pull-left8[
### `.RData`  
+ Stores bundle several independent R objects.
...
Under the hood practically all reading and writing functions rely on R's file connection architecture, which is very similar to most other programming languages. To the savvy programmer represent a flexible option access and manipulate files on any accessible location (harddrive, server, www) is file connections. 
