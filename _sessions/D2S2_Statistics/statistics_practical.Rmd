...
27. A scientist wants to know whether there was an effect of treatment arm on the probability that patients had more than 1000 days before the first occurrence of a major negative event. Answer this question using a logistic regression. (Hint: To do this, you should first create a new column called `drugs_1000_bin` that is a binary variable indicating whether or not `days > 1000`. You can do this using `mutate()`)
...
32. Only for patients older than 40, is there a relationship between antiviral history (`str2`) and race (`race`)?
