...
---
## Managing working directories in projects
...
When you knit a Markdown file, the working directory will be temporarily changed to the location of that file.

This can cause issues when your Markdown file is not in the root directory of your project, but in a sub-folder.
...
2 Solutions to the working directory issue

1: Include `knitr::opts_knit$set(root.dir = '')` in your first chunk. This will move the working directory when the document is knitted.

2: Use the `here()` function from the `here` package. This always returns the project root directory.
...
```{r, eval = FALSE}
# Root directory is one folder up
...
```{r, eval = FALSE}
# Read in data relative to root directory
mydata <- read_csv(file = paste0(here(), "/data/mydata.csv"))
```
