...
C4. So far we've used base plotting functions (e.g.; `hist()`, `abline()`). Try replacing these functions with ggplot code. For example, you could use `ggplot(data, aes(x)) + geom_hist()` to create a histogram. Note that because of the way you build on plots in ggplot, using the `addmean` functionality will be a bit tricky, so you may wish to remove it.
