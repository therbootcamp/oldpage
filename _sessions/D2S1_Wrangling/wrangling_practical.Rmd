...
2. Oops, you just learned that your data repository doesn't allow underscores in column names. Change `age_years` to `agey`

### `mutate()`

3. Add a new column to `trial_act` called `agem` that shows each patient's agey in months instead of years (Hint: do simple arithmetic!).
...
6. A physician wants to see a new score called the `sparrow` which is equal to the Karnofsky score divided by a person's agey plus the person's weight in kg. Add each participant's sparrow score as a new column to `trial_act` called `sparrow` (Hint: Just take `karnof / agey + weightkg)
...
14.  Create a new tibble called `trial_act_C` that only contains data from people who have not used intraveneous drugs (`drugs == 0`) *and* are over 40 years old *and* are female
...
26. This is the `diamonds` dataframe, which is included in the `dplyr` package. Print the `diamonds` data to see how it looks. Then, using the `diamonds` data, for each value of `cut` and `color` (i.e.; group the data by these two variables), calculate the minimum, mean, and maximum price. Then, arrange the data by mean price (in descending order). Store the result as a new tibble called `diamonds_cut_agg`. What combination of cut and color is at the top of the data as having the highest mean price?
