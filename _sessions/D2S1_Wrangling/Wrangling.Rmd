...
Anytime you want to transform, organise, or aggregate data, use `dplyr`
...
data %>%           # Start with data, and THEN
     FUNCTION1 %>%     # Do FUNCTION1, (and THEN)
     FUNCTION2 %>% ... # Do FUNCTION2, (and THEN)
```
...
scores_df <- tibble(id = c(67, 5, 34, 13, 22),
                    time1 = c(2, 3, 3, 4, 7),
                    time2 = c(4, 3, 5, 6, 5))
```
...
<font size = 5>Goal: Add scores_df data to patents_df</font>
<br3>
<font size = 5> For each matching id in scores, add time1 and time2 to patients_df
...
<font size = 5>Must include one or more 'key' columns used to match rows<br><br></font>
<font size = 5>Example:<br><br></font>
...
.pull-right65[
<font size = 5>4) Merge patients_df with scores_df</font>
...
# 2) Create new columns with mutate()
  ...
  # 3) Add data from scores_df with left_join()
...
.pull-right65[
<br><br>
<font size = 5>patients_df <- patients_df %>% ...</font>
...
- Select drugX over the age of 30
...
.pull-right65[
<font size = 5>1) Sort rows by id then in decending order of age</font>
...
<font size = 5>The filter() funciton is used to select specific rows</font>
<br><br>
...
<font size = 5>Within summarise() create new variables using summary functions<font>
<br>
