...
### (advanced) Speeding up code: Parallel R
...
### (advanced) Speeding up code: R Open

Another option to speed up certain parts of R, in particular anything related to linear algebra (e.g., matrix multiplication), is to use a different implementation R. One such implementation is Microsoft Open R. Open R features a high-performance, multi-threaded [BLAS/LAPACK](http://www.netlib.org/lapack/lug/node11.html) linear algebra libraries. Another feature of Open R is reproducibility. With every version of, Open R freeezes its basic collection of packages in a repository in order to avoid any issues arising from package versioning. 

In this exercise, you will be comparing to chunks of code in R and Open R to see where the performance benefits of Open R lie and where not. To do this first install [Open R](https://mran.microsoft.com/open). This will create a separate R instance called Open R. Start it and run the code examples below, once in Open R and once in your regular R. You can see that one of the examples runs dramatically faster, although it is already very efficiently implemented in regular R. That the other one, however, isn't any faster implies that Open R provides hardly any benefits for regular users of R.        
...
### (advanced) Speeding up code: Rcpp

A universal option to speed up run time in R is to implement essential code chunks in C++ using Rcpp. C++ is a compiled language, meaning that any C++ code will be translated in to highly efficient machine code, before its execution. For this reason, implementing R code in C++ and importing it using Rcpp regularly means 10-100x faster code execution. When using high performance libraries such as, e.g., [RcppArmadillo](),      
